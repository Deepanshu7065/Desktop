...
export const RepairAddPrice = () => {
    const queryClient = useQueryClient()
    const repairAddPrice = async ({ data }: any) => {
        try {
            const response = await axios.post(`${baseUrl}/price/add`, data)
            return response
        } catch (error) {
            console.log(error)
        }
...
export const DeleteRepairPriceData = () => {
    const queryClient = useQueryClient()
    const deleteRepairPriceData = async ({ id }: any) => {
        try {
            const response = await axios.delete(`${baseUrl}/price/delete/${id}`)
            return response
        } catch (error) {
            console.log(error)
        }
    ...
    return useMutation({
        mutationFn: deleteRepairPriceData,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["repair"] })
        }
    })
